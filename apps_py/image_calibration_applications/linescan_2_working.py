...
def scan_image(image_array,red_image,blue_image,green_image):
    ...
    red_image : a 2D nd.numpy array that only contains the red channel of the orginal image.
    ...
    BLUE_FILTER_LOC     = 200 #pixel row where the blue filter is applied 
    GREEN_FILTER_LOC    = 400 # pixel row where the green filter is applied
    
    image_height = image_array.shape[0]
    image_width = image_array.shape[1] 
    
    updating_image = np.zeros((image_height, image_width, 3), dtype=np.uint8)
    ...
    for current_row_index in range(2300,-400, -1):
        ...
        new_frame = Image.fromarray(updating_image)
        frames.append(new_frame)
    
    output_file_name = get_out_file_name()    
    output_path = r"C:\github\image_processing_python\linescan_animations\ " + output_file_name + ".mp4"
...
def get_out_file_name():
    return input("Enter the output file name : ")
...
def main():
    ...
    #split the image into seperate channels and then display it 
